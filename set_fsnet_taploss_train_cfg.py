...
def save_train_path(noisy_files, clean_files):
    ...
    # 'w': 덮어쓰기, 'a': 이어쓰기
    with open('/content/train_noisy_paths.txt', 'w') as f:
        for line in noisy_paths:
            f.write(line)
            f.write('\n')

    with open('/content/train_clean_paths.txt', 'w') as f:
...
def save_train_config(train_toml):
    filename = '/content/TAPLoss-master/FullSubNet/recipes/dns_interspeech_2020/fullsubnet/taploss_newdata_005_lr00001.toml'
    print(filename)
...
def main():
    ...
    train_toml="""
        ...
        [acoustic_loss]
        ac_loss_weight = 0.05 # change to 1 to use acoustic loss with weight 1
        ac_loss_only  = false
        ...
        [train_dataset.args]
        clean_dataset = "/content/train_clean_paths.txt"
        ...
        noisy_dataset = "/content/train_noisy_paths.txt"
